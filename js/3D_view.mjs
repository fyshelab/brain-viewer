...
export async function initScene({ dom_elem }) {
  ...
  //let width = 0.75;
  //let height = 0.6;
  //const camera = new THREE.OrthographicCamera(-width, width, height, -height, 0.0, 1000)
  const renderer = new THREE.WebGLRenderer({ canvas: dom_controls.canvas, antialias: true });
...
export async function add_brain({
  ...
  let brain_data = await load_all_promises({
    pt: load_npy(pt_flat),
    faces_flat: load_npy(faces_flat),
    faces: load_npy(faces_inflated),
    pt2: load_npy(pt_inflated),
    pt3: load_npy(pt_pia),
    pt4: load_npy(pt_wm),
    counts_data: (await fetch(counts)).json(),
    curvature: cached_load_npy(curvature_path),
    cached_map: load_npy(mapping_path),
  });
  ...
  async function set_texture(data, width, height) {
    ...
    if (show_roi) {
      let foreground = await get_png_data("static_data/foreground.png");
      data = overlay_images_uint8(data, foreground, width, height);
      // Usage
      //try {
      //  data = await addSvgPathToImage(mapping_path, data, width, height);
      //} catch (e) {
      //  console.error(e);
      //}
    }
...
async function addSvgPathToImage(svgUrl, data, width, height) {
  // Load the SVG file
