export async function load_npy(url) {
...
export function concat_npy(arrays) {
...
export async function get_png_data(url) {
...
export function overlay_images_uint8(baseArray, overlayArray, width, height) {
...
export async function cached_load_npy(url) {
  if (url in cached) {
    return cached[url];
  }
  const data = await load_npy(url);
  cached[url] = data;
  return data;
}


export async function load_all_promises(promises) {
  let result = {};
  let promise_list = [];
